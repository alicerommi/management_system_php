    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
             ...
             <?php
             // echo" SELECT* FROM `packages` WHERE `ins_id`=$ins_ID"; die;
              $packageIDS = array();
              $userIDS = array();
              $bookingIDS = array();
              // $queryPackages = mysqli_query($conn,"SELECT packages_booking.package_id, packages_booking.user_id,packages_booking.package_booking_id,
              // FROM packages_booking
              // LEFT JOIN packages ON packages_booking.package_id = packages.package_id");
              $queryPackages = mysqli_query($conn,"SELECT packages_booking.package_id, packages_booking.user_id,packages_booking.package_booking_id,packages_booking.package_requestStatus FROM packages_booking LEFT JOIN packages ON packages_booking.package_id = packages.package_id");

              //check point to know the package request status
              //$rowPackCheck = mysqli_fetch_array($queryPackages);
              //$check = $rowPackCheck['package_requestStatus'];
             // $rows = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM packages_booking WHERE `package_requestStatus`='1'"));
          
              if(mysqli_num_rows($queryPackages)>0){

                            while($rowPack = mysqli_fetch_array($queryPackages)){
                              if($rowPack['package_requestStatus']=="0"){
                                // echo $rowPack['package_requestStatus'];
                                // die;
                                array_push($packageIDS, $rowPack['package_id']);
                                array_push($userIDS, $rowPack['user_id']);
                                array_push($bookingIDS,$rowPack['package_booking_id']);
                              }//end inner condition
                             
                            }//end while loop 
                }//end if condition
              if(sizeof($packageIDS)>0){ ?>
              <button id="buttonHiders" class="btn btn-default input-sm">X</button>
            <div class="control-group" id="removeAssign"> 
                  <label class="control-label" for="inputPatient">Assign Slot To Learner:</label>
                  <select class="form-control assignLearner">
                     <option selected>Nothing Selected</option>
                     <?php 
                     for($i=0;$i<sizeof($bookingIDS);$i++){ 
                                $user_id = $userIDS[$i];
                                $package_booking_id = $bookingIDS[$i];
                                //get user name from users table
                                $queryUser = mysqli_query($conn,"SELECT* FROM `users` WHERE `id`=$user_id");
                                $userRow = mysqli_fetch_array($queryUser);
                                $userName = $userRow['username'];
                                //here the value is actually userid, name is the package_booking_id used in packages_booking table
                                echo '<option value='.$user_id.' name='.$package_booking_id.'>'.$userName.'</option>';      
                      }//end for loop here
                      ?>
                  </select>
              </div>   
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
       ...
       //for assinging the slot

        function doSubmit1(id){
         var  booking_package_id = id;
          ...
          $.ajax({
               ...
               url: 'addTiming.php',
                data: 'action=addPackage&userID='+userID+'&booking_package_id='+booking_package_id+'&start='+startTime+'&end='+endTime+"&title="+title,
                type: "POST",
                success: function(json) {
                    $("#calendar").fullCalendar('renderEvent',
                    {
...
<script type="text/javascript">
  ...
  //for hiding the div if instrucotr dont want to any learner who has make request for package
  $(document).ready(function(){
    $('#buttonHiders').click(function(){
      $('#removeAssign').hide();
      $(this).hide();
