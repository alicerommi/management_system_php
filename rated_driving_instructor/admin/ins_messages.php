...
<script>
 ...
 {
      $("#messages").load("chatFiles/load.php?admin_id="+senderID+"&ins_id="+recieverID).show();
       scrollNow();
            ...
            <div class="container-fluid">
                <div class="row bg-title">
                    ...
                    <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12"> 
                        <ol class="breadcrumb">
                            ...
                            <li class="active">Instructor Messages</li>
                        </ol>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                ...
                <div class="row">
                            ...
                            <div class="col-lg-3 b-r" id="users">
                               <center> <h3>All Instructors</h3></center><hr/>
                                   <?php 
                                   //to get the instructor id 
                                   $query = mysqli_query($conn,"SELECT* FROM `admin` WHERE email='$admin_email'");
                                        $row = mysqli_fetch_array($query);
                                       // $instructorName = $row['name'];
                                        $adminID= $row['id']; //which is the insteructor id 
                                             //to get the all the learner 
                                        echo '<input type="hidden" id="adminID" value='.$adminID.'>';
                                   $query = mysqli_query($conn,"SELECT * FROM instructor WHERE emailVeriStatus = 1");
                                   if(mysqli_num_rows($query)>0){
                                            while($insRow = mysqli_fetch_array($query)){
                                              //get the instructor details
                                              $ins_id = $insRow['id'];
                                              $ins_name = $insRow['name'];
                                              $ins_email = $insRow['email'];
                                              $online_flag = $insRow['online_flag'];
                                               $insImage  = $insRow['image'];
                                                 //attach a green color if the instructor is online using the flag value
                                                $str = "";
                                                 if($online_flag!=0){
                                               ...
                                               // echo "<ul>";
                                                ...
                                                if($insImage==""){
                                                    echo "<a class='usersClicked' id='$ins_id' name='$ins_name' alt='$ins_name' type='button'><img class='my_profile_image' src='../user/images/default.png' height='35' width='35'/>&nbsp;&nbsp;"."<strong>".ucwords($ins_name)."</strong>".$str."</a><hr/>";
                                                }else{
                                                    echo "<a class='usersClicked' id='$ins_id' name='$ins_name' alt='$ins_name'><img class='my_profile_image' src='../user/images/$userImage' height='35' width='35'/>&nbsp;&nbsp;"."<strong>".ucwords($ins_name)."</strong>".$str."</a><hr/>";
                                                }
                                              //  echo "</ul>";
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on('click','.usersClicked',function(){
                //get user id 
               var instructorID = $(this).attr('id');
               //get instructor id
               var adminID = $("#adminID").val();
               //for sending request to page 
               var dataString = "ins_id="+instructorID+"&admin_id="+adminID;
               $.ajax({
